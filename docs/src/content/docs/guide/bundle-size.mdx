...
`@qubee/core` has **zero runtime dependencies**. What you ship depends on one decision: whether you
...
```ts
import { STRAPI_DRIVER, QubeeStore, QueryBuilder } from '@qubee/core';
```
...
```ts
import { DRIVERS } from '@qubee/core';
