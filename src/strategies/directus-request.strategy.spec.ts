  ...
  describe('operator filters', () => {
    ...
    it('should emit IN as _in CSV', () => {
      const state = {
        ...baseState,
        operatorFilters: [
          { field: 'tag', operator: FilterOperatorEnum.IN, values: ['a', 'b', 'c'] },
        ],
      };
      const uri = strategy.buildUri(state, options);
    ...
    it('should throw InvalidFilterOperatorValueError when NULL receives a non-boolean', () => {
      const state = {
        ...baseState,
        operatorFilters: [
          { field: 'deletedAt', operator: FilterOperatorEnum.NULL, values: ['yes'] },
        ],
      };
  ...
  describe('combined queries', () => {
    it('should build a complete Directus query URI', () => {
      const state: QueryBuilderState = {
        ...
        includes: ['author'],
        limit: 10,
        operatorFilters: [{ field: 'likeCount', operator: FilterOperatorEnum.GTE, values: [100] }],
        page: 2,
        resource: 'articles',
        select: ['title'],
