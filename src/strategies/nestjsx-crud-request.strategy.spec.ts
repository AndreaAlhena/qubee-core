...
describe('NestjsxCrudRequestStrategy', () => {
  ...
  describe('operator filters', () => {
    ...
    it('should emit CONTAINS as $cont', () => {
      const state = {
        ...baseState,
        operatorFilters: [
          { field: 'name', operator: FilterOperatorEnum.CONTAINS, values: ['foo'] },
        ],
      };
      const uri = strategy.buildUri(state, options);
    ...
    it('should emit NOT with a single value as $ne', () => {
      const state = {
        ...baseState,
        operatorFilters: [
          { field: 'status', operator: FilterOperatorEnum.NOT, values: ['archived'] },
        ],
      };
      const uri = strategy.buildUri(state, options);
    ...
    it('should emit NOT with multiple values as $notin', () => {
      const state = {
        ...baseState,
        operatorFilters: [
          { field: 'status', operator: FilterOperatorEnum.NOT, values: ['archived', 'draft'] },
        ],
      };
      const uri = strategy.buildUri(state, options);
    ...
    it('should emit NULL=true as bare $isnull with no value segment', () => {
      const state = {
        ...baseState,
        operatorFilters: [
          { field: 'deleted_at', operator: FilterOperatorEnum.NULL, values: [true] },
        ],
      };
      const uri = strategy.buildUri(state, options);
    ...
    it('should emit NULL=false as bare $notnull with no value segment', () => {
      const state = {
        ...baseState,
        operatorFilters: [
          { field: 'deleted_at', operator: FilterOperatorEnum.NULL, values: [false] },
        ],
      };
      const uri = strategy.buildUri(state, options);
    ...
    it('should throw InvalidFilterOperatorValueError when NULL receives a non-boolean', () => {
      const state = {
        ...baseState,
        operatorFilters: [
          { field: 'deleted_at', operator: FilterOperatorEnum.NULL, values: ['yes'] },
        ],
      };
  ...
  describe('custom key names', () => {
    ...
    it('should keep filter, join, sort, and fields hardcoded regardless of options', () => {
      const customOptions = new QueryBuilderOptions({
        filters: 'flt',
        includes: 'inc',
        select: 'sel',
        sort: 'srt',
